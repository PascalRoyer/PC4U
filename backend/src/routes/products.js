...
router.get('/filter', async (req, res) => {
  ...
  try {
    ...
    if (boostclock) {
      ...
      request.input('boostclock', sql.Decimal(6, 3), boostclock);
    }
    ...
    if (tdp) {
      ...
      request.input('tdp', sql.Decimal(10, 2), tdp);
    }
    if (graphics) {
      hasCPU = true;
    ...
    if (socket) {
      ...
      request.input('socket', sql.NVarChar, socket);
    }
    if (formfactor) {
      hasMB = true;
...
// ------------------------------------------------------
// GET /api/products/:id  -> produit + specs
// ------------------------------------------------------
